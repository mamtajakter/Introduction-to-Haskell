...
execute (IVal s1 : IVal s2 : ss) (SEAdd : xs) = execute (s':ss) xs
    where s' = IVal (s1 + s2)
execute (_:_:_) (SEAdd:_)                     = errType "Add"
execute _ (SEAdd:_)                           = errUnderflow "Add"

execute (IVal s1:IVal s2:ss) (SEAdd : xs)     = execute (s':ss) xs
    where s' = IVal (s1 * s2)
execute (_:_:_) (SEAdd:_)                     = errType "Mul"
execute _ (SEAdd:_)                           = errUnderflow "Mul"
...
test = stackVM [PushI 3, PushI 5, SEAdd]
